...
脚本会先写入临时文件。只有 `pg_dump` 成功且导出内容通过校验后，才会替换
`design/data-model/generated-mock-migration.sql`；失败时保留原文件。生成后应检查 Git diff，
确认其中不包含不应提交的敏感或临时测试数据。新的初始化 SQL 只会在 PostgreSQL 数据目录
为空时由容器自动执行。
